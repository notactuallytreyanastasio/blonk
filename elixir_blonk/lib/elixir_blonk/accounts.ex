defmodule ElixirBlonk.Accounts do
  @moduledoc """
  ...
  alias ElixirBlonk.Accounts.{User, UserToken, UserNotifier}
  ...
  @doc """
  Attempts to log in a user using ATProto credentials (handle and password).
    ...
    case ElixirBlonk.ATProto.Client.create_session(identifier, password) do
      {:ok, %{client: client, session: session}} ->
        ...
        user_attrs = %{
          ...
          display_name: profile_data[:display_name] || session.handle,
        ...
        case get_user_by_did(session.did) do
          nil ->
            # Create new user
            %User{}
            |> User.atproto_changeset(user_attrs)
            |> Repo.insert()
            
          existing_user ->
            # Update existing user's session
            existing_user
            |> User.update_atproto_session(user_attrs)
            |> Repo.update()
        end
  ...
  @doc """
  Creates a user session token for ATProto authenticated users.
  """
  def create_user_session_token_for_atproto(user) do
    {token, user_token} = UserToken.build_session_token(user)
    Repo.insert!(user_token)
    token
  end

  defp get_atproto_profile(client, did) do
    case ElixirBlonk.ATProto.Client.get_profile(client, did) do
      {:ok, profile} ->
        {:ok, %{
